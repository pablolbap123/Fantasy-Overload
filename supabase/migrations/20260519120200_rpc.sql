...
begin
  ...
  select
    v_league_id,
    id,
    case when row_number() over (order by current_price desc, id) <= 10 then 'market' else 'locked' end,
    current_price,
    round((current_price * 1.8) / 50000) * 50000,
    case when row_number() over (order by current_price desc, id) <= 10 then now() else null end,
    case when row_number() over (order by current_price desc, id) <= 10 then now() + interval '24 hours' else null end
  from public.players;
