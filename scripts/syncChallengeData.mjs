...
const players = rawPlayers.map((player, index) => {
  const team = teamByChallengeId.get(player.competitorId) ?? teamByShortName.get("BLSA");
  const position = derivePosition(player, stageRoom.lineupPositions, index);
...
for (const player of players) {
  ...
  const team = teams.find((item) => item.id === player.teamId) ?? teamByShortName.get("BLSA");
  const seed = hashNumber(player.challengeId);
  player.currentPrice = priceFor(team, player.position, player.totalPoints, player.stats, seed);
  player.basePrice = player.currentPrice;
}
